...
class DizainesWindow(Gtk.Window):
    ...
    def __init__(self):
        ...
        if os.path.exists('.git'):
            # On est dans le git
            self.dirBase = './source/'
            self.p.dprint("Dans le git")
            
        else:
            if os.path.isdir('/usr/share/dizaines/') == True:
                # Sur install paquet
                self.dirBase = '/usr/share/dizaines/'
                self.p.dprint("Via deb package")
            else:
                # en dev
                self.dirBase = './'
                self.p.dprint("Dans le DEV")
        ...
        for j in range(1,5):
            for i in range(0,5):
                pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(filename=self.dirBase + "images/0.png", width=100, height=100,
    ...
    def traitement_cartes(self, button, value, stage):
        ...
        if value not in self.choix_tous_les:
            ...
            if len(self.choix_cases) == 2:
                pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(filename=self.dirBase + "images/R" + value,
                ...
                if len(self.choix_cases) == 4:
                    if self.choix_cases[1][1:-4] == self.choix_cases[3][1:-4]:
                        ...
                        pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(filename=self.dirBase + "images/0.png",
                                                                         width=100, height=100,
                                                                         preserve_aspect_ratio=True)
                        img = Gtk.Image.new_from_pixbuf(pixbuf)
                        ...
                        # On efface les cartes deja jouées
                        pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(filename=self.dirBase + "images/error.png",
                                                                         width=100, height=100,
                                                                         preserve_aspect_ratio=True)
                        img = Gtk.Image.new_from_pixbuf(pixbuf)
                        img2 = Gtk.Image.new_from_pixbuf(pixbuf)
    ...
    def charger_jeu(self, button):
        ...
        for i in range(0,20):
            if self.niveau == 1:
                ...
                pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(filename=self.dirBase + "images/"+self.list_of_random_items[pos],
                                                                 width=100, height=100,
                                                                 preserve_aspect_ratio=True)
                img = Gtk.Image.new_from_pixbuf(pixbuf)
